...
router.get('/', function (req, res, next) {
    ...
    if (uuid == undefined) {
        ...
        var dataString = 'grant_type=authorization_code&redirect_uri=http%3A%2F%2Flocalhost%3A3000%2F&code=' + uuid + '&client_id=16g26706ovmhfj4d3o08qnnso';

        var options = {
            url: 'https://ouichef.auth.us-east-2.amazoncognito.com/oauth2/token',
            method: 'POST',
            headers: headers,
            body: dataString
        };
        ...
        function callback(error, response, body) {
            if (!error && response.statusCode == 200) {
                ...
                token = JSON.parse(body).id_token;
